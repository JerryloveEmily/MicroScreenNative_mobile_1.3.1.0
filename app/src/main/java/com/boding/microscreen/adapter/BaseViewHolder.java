...
public class BaseViewHolder {
	...
	protected BaseViewHolder(Context context, int layoutResId, ViewGroup parent, int position){
	...
	public static BaseViewHolder get(Context context,View convertView, int layoutResId, ViewGroup parent){
		return get(context, convertView, layoutResId, parent, -1);
	}
	
	protected static BaseViewHolder get(Context context, View convertView, int layoutResId, ViewGroup parent, int position){
		if (convertView == null) {
			return new BaseViewHolder(context, layoutResId, parent, position);
		}
		
		BaseViewHolder existHolder = (BaseViewHolder) convertView.getTag();
		if (existHolder.layoutResId != layoutResId) {
			// 不同的item布局设置不同的 ViewHolder
			return new BaseViewHolder(context, layoutResId, parent, position);
		}
		
		existHolder.position = position;
		return existHolder;
	}
